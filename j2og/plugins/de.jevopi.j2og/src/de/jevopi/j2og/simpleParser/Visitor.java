 ...
 ******************************************************************************/
package de.jevopi.j2og.simpleParser;
...
public class Visitor extends ASTVisitor {
		...
		if (binding != null) {
			...
			if (!binding.isInterface()) {
				...
				if (_superBinding != null) {
					((Class) type).setSuper((Class) getType(_superBinding
						.getErasure(), true));
				}
			}
	...
	private Type getType(ITypeBinding typeBinding, boolean isContext) {
		...
		if (t == null) {
			...
			if (typeBinding.isInterface()) {
				...
				((Class) t).setAbstract(Modifier.isAbstract(typeBinding
					.getModifiers()));
			}
	...
	public boolean visit(SimpleName i_node) {
		...
		if (!classifierStack.isEmpty()) {
			...
			if (typeBinding != null) {
				...
				if (type != classifierStack.peek()
					&& !type.getFqn().startsWith(
							classifierStack.peek().getFqn())) {
	...
	public boolean visit(TypeDeclaration i_node) {
		if (i_node.isLocalTypeDeclaration()) return false;
		return startType(i_node);
	}
	...
	public void endVisit(TypeDeclaration i_node) {
		...
		if (i_node.isLocalTypeDeclaration()) return;
		endType(i_node);
	}
...
//	}
	 ...
	 * @since Aug 19, 2011
	 */
	private void initTypedElement(TypedElement typedElement,
	...
	private Scope getScope(int i_modifiers) {
		if (Modifier.isPrivate(i_modifiers)) return Scope.PRIVATE;
		if (Modifier.isProtected(i_modifiers)) return Scope.PROTECTED;
		if (Modifier.isPublic(i_modifiers)) return Scope.PUBLIC;
	...
	private boolean isCollection(ITypeBinding i_varTypeBinding) {
		...
		for (ITypeBinding _interface : i_varTypeBinding.getInterfaces()) {
			if (isCollection(_interface)) return true;
		}
		return false;
	}
	...
	public boolean visit(MethodDeclaration i_node) {
		...
		if (binding != null && !binding.isConstructor()) {
			...
			for (int i = 0; i < i_node.parameters().size(); i++) {
				VariableDeclaration parNode =
					(VariableDeclaration) i_node.parameters().get(i);
